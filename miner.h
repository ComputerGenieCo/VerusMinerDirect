...
// Critical macros that must be defined early
#define USER_AGENT PACKAGE_NAME "/" PACKAGE_VERSION

// JSON compatibility macros - must be defined after jansson.h
...
#define MAX_GPUS 140
	...
	// Basic structs without dependencies
	struct tx
	{
	...
	// Full struct definitions in dependency order
	struct stratum_job
	{
		// Job identification
		char *job_id;
	...
	struct stratum_ctx
	{
		// Connection info and state - arrange fields to minimize padding
		char *url;
		char *curl_url;
		char *sockbuf;
		char *session_id;
...
// Pool status flags - add with other pool definitions
#define POOL_ST_DEFINED 1
#define POOL_ST_VALID 2
#define POOL_ST_DISABLED 4
#define POOL_ST_REMOVED 8

	// Forward declarations for structs with private implementations
	struct thread_q;
	...
	// Pool externals - add with other extern declarations
...
#define EXIT_CODE_KILLED 7
	...
	void print_hash_tests(void); // Fixed missing parenthesis

// Pool related declarations - add before struct definitions
#define POOL_UNUSED 0
#define POOL_GETWORK 1
#define POOL_STRATUM 2
#define POOL_LONGPOLL 4

	// Network function declarations - add in function declarations section
	double verus_network_diff(struct work *work);

	// Pool management declarations
